...
#  Loop through each tweet
    for tweet in extracted_tweets:
        ...
        ## Sending above Prompt to LLM
        out = execute_gemini(sentiment_analysis_prompt)
        out_dict = json.loads(out)  # Converting  json String To python Code
        print(out)
        out_dict["tweet"] = tweet["text"] 
        analyzed_tweets.append(out_dict)

    with open("analyzed_tweets.json", "w") as analyzed_tweets_file:
        json.dump(analyzed_tweets, analyzed_tweets_file)   # Dumping Whole List into Json File 
